...
let player;
let gamePush;
const GAMEPUSH_CONFIG = { appId: 'YOUR_GAMEPUSH_APP_ID', enabled: true };
...
const translations = {
    en: {
        ...
        toast_prestige_done_v2: "Prestige! Level {level}! Earned {points} PP (Total: {total})",
    ...
    ru: {
        ...
        toast_prestige_done_v2: "Престиж! Уровень {level}! Получено {points} ОП (Всего: {total})",
...
const skillsTreeData = [
    {
        ...
        name: { ru: "Мастерство Клика", en: "Click Mastery" },
        description: { 
            ru: "Увеличивает силу клика на 50% постоянно", 
            en: "Permanently increases click power by 50%" 
        },
        cost: 5,
        effect: { type: "clickMultiplier", value: 1.5 },
        requires: [],
        icon: "⚡"
    },
    {
        id: "skill_synthesis_boost",
        name: { ru: "Эффективный Синтез", en: "Efficient Synthesis" },
        description: { 
            ru: "Увеличивает пассивный доход на 50% постоянно", 
            en: "Permanently increases passive income by 50%" 
        },
        cost: 5,
        effect: { type: "autoMultiplier", value: 1.5 },
        requires: [],
        icon: "⚙️"
    },
    {
        id: "skill_crit_mastery",
        name: { ru: "Критический Удар", en: "Critical Strike" },
        description: { 
            ru: "Увеличивает шанс крита на 5% и множитель на 0.5x", 
            en: "Increases crit chance by 5% and multiplier by 0.5x" 
        },
        cost: 8,
        effect: { type: "critBoost", chance: 0.05, multiplier: 0.5 },
        requires: ["skill_click_boost"],
        icon: "🎯"
    },
    {
        id: "skill_prestige_bonus",
        name: { ru: "Наследие Предков", en: "Ancestral Legacy" },
        description: { 
            ru: "Увеличивает бонус престижа на 25%", 
            en: "Increases prestige bonus by 25%" 
        },
        cost: 10,
        effect: { type: "prestigeMultiplier", value: 1.25 },
        requires: ["skill_synthesis_boost"],
        icon: "🏆"
    },
    {
        id: "skill_omnipotence",
        name: { ru: "Всемогущество", en: "Omnipotence" },
        description: { 
            ru: "Все бонусы x2 (требует все предыдущие навыки)", 
            en: "All bonuses x2 (requires all previous skills)" 
        },
        cost: 25,
        effect: { type: "globalMultiplier", value: 2.0 },
        requires: ["skill_crit_mastery", "skill_prestige_bonus"],
        icon: "👑"
    }
];
...
const relicsData = [
    {
        id: "relic_quantum_crystal",
        name: { ru: "Квантовый Кристалл", en: "Quantum Crystal" },
        description: { 
            ru: "+100% к энергии за клик", 
            en: "+100% energy per click" 
        },
        cost: 10,
        effect: { type: "clickBonus", value: 2.0 },
        icon: "💎"
    },
    {
        id: "relic_stellar_core",
        name: { ru: "Звёздное Ядро", en: "Stellar Core" },
        description: { 
            ru: "+100% к пассивному доходу", 
            en: "+100% passive income" 
        },
        cost: 10,
        effect: { type: "autoBonus", value: 2.0 },
        icon: "⭐"
    },
    {
        id: "relic_time_fragment",
        name: { ru: "Осколок Времени", en: "Time Fragment" },
        description: { 
            ru: "Авто-клики работают в 2 раза быстрее", 
            en: "Auto-clicks work 2x faster" 
        },
        cost: 15,
        effect: { type: "autoclickSpeed", value: 2.0 },
        icon: "⏳"
    },
    {
        id: "relic_multiverse_key",
        name: { ru: "Ключ Мультивселенной", en: "Multiverse Key" },
        description: { 
            ru: "Удваивает очки престижа при сбросе", 
            en: "Doubles prestige points on reset" 
        },
        cost: 20,
        effect: { type: "prestigeDouble", value: 2.0 },
        icon: "🔑"
    },
    {
        id: "relic_evolution_essence",
        name: { ru: "Сущность Эволюции", en: "Evolution Essence" },
        description: { 
            ru: "Все бонусы +50% (требует все предыдущие реликвии)", 
            en: "All bonuses +50% (requires all previous relics)" 
        },
        cost: 50,
        effect: { type: "globalBonus", value: 1.5 },
        requires: ["relic_quantum_crystal", "relic_stellar_core", "relic_time_fragment", "relic_multiverse_key"],
        icon: "🧬"
    }
];
...
function gameTick() {
    ...
    // Применяем множители от навыков
    if (gameState.purchasedSkills) {
        for (const skillId of gameState.purchasedSkills) {
            const skill = skillsTreeData.find(s => s.id === skillId);
            if (skill && skill.effect) {
                if (skill.effect.type === "autoMultiplier") {
                    energyPerSec *= skill.effect.value;
                } else if (skill.effect.type === "globalMultiplier") {
                    energyPerSec *= skill.effect.value;
                }
            }
        }
    ...
    // Применяем множители от реликвий
    if (gameState.ownedRelics) {
        for (const relicId of gameState.ownedRelics) {
            const relic = relicsData.find(r => r.id === relicId);
            if (relic && relic.effect) {
                if (relic.effect.type === "autoBonus") {
                    energyPerSec *= relic.effect.value;
                } else if (relic.effect.type === "globalBonus") {
                    energyPerSec *= relic.effect.value;
                }
            }
        }
...
function performClick(e, isAuto) {
    ...
    // Применяем бонус престижа к силе клика
    clickPower *= 1 + 0.1 * gameState.prestigeClickLevel;
    ...
    if (gameState.purchasedSkills) {
        for (const skillId of gameState.purchasedSkills) {
            const skill = skillsTreeData.find(s => s.id === skillId);
            if (skill && skill.effect) {
                if (skill.effect.type === "clickMultiplier") {
                    clickPower *= skill.effect.value;
                } else if (skill.effect.type === "globalMultiplier") {
                    clickPower *= skill.effect.value;
                }
            }
    ...
    if (gameState.ownedRelics) {
        for (const relicId of gameState.ownedRelics) {
            const relic = relicsData.find(r => r.id === relicId);
            if (relic && relic.effect) {
                if (relic.effect.type === "clickBonus") {
                    clickPower *= relic.effect.value;
                } else if (relic.effect.type === "globalBonus") {
                    clickPower *= relic.effect.value;
                }
            }
...
function executePrestigeLogic() {
    ...
    // НОВАЯ ФОРМУЛА: Расчет очков престижа на основе текущей энергии
    const basePoints = Math.max(1, Math.floor(Math.sqrt(gameState.energy / 1000000)));
    
    // Множитель от реликвии "Ключ Мультивселенной"
    let multiplier = 1.0;
    if (gameState.ownedRelics && gameState.ownedRelics.includes("relic_multiverse_key")) {
        multiplier *= 2.0;
    }
    
    const earnedPrestigePoints = Math.floor(basePoints * multiplier);

    // Сбрасываем состояние игры, но сохраняем улучшения престижа и настройки
    gameState = createDefaultGameState();
    gameState.prestigeLevel = newPrestigeLevel;
    gameState.prestigePoints = oldState.prestigePoints + earnedPrestigePoints;
    ...
    // Сохраняем купленные навыки и реликвии
    gameState.purchasedSkills = oldState.purchasedSkills || [];
    gameState.ownedRelics = oldState.ownedRelics || [];
    ...
    // Сообщение о полученных очках престижа
    const toastMsg = getLangText("toast_prestige_done_v2", { 
        level: newPrestigeLevel, 
        points: earnedPrestigePoints,
        total: gameState.prestigePoints 
    });
